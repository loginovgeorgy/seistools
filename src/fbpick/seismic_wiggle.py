...
def wiggle(
    ...
    for ntr in range(Ntr):
        ...
        for i, label in enumerate(picks):
            p = picks[label]
            ax.plot(offset + trace[np.int32(p[ntr])], p[ntr] * np.ones(1),
                    # color=plt.cm.tab10(i),
                    markeredgecolor=plt.cm.tab10(i),
    ...
    if picks:
        for i, label in enumerate(picks):
            if curve:
                ...
                y = np.arange(Ntr) + traces[p, np.arange(Ntr)]
                ax.plot(y, picks[label],
                        color=plt.cm.tab10(i),
                        label=label,
                        markeredgecolor=plt.cm.tab10(i),
                        markerfacecolor='None',
                ...
                ax.plot(np.nan, np.nan, color=plt.cm.tab10(i), label=label,
                        markeredgecolor=plt.cm.tab10(i),
                        markerfacecolor='None',
                        marker=MARKERS[i],
                        linestyle='None'
