...
def RT_Coefficients(σ1, σ2, Vp1, Vs1, Vp2, Vs2, Wave_Type, Angle_Deg):
...
# поляризации отражённых и преломлённых волн:
    ...
    U = Polarizations(Cij2, pTransP) # находим, с какими поляризациями волна может распространяться
    # в 1-й среде при заданном веторе медленности pReflP
    
    UTransP = U[:, 0] #по построению, поляризация продольной волны - "первая в списке".
    
    U = Polarizations(Cij2, pTransS1)
